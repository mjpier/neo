...
unset alphabet btrfs_raid disk disks fallback_password first_partition_start index integer_regex luks_device luks_password partition raid short_uuid swap_partition swap_size system_size token_id token_type
...
while getopts 56b:d:e:f:s:th opt; do
    case $opt in
        5) setRaid 5;;
        6) setRaid 6;;
...
if { [[ -n ${raid} ]] && [[ ${raid} -eq 5  ]] && [[ ${#disks[@]} -lt 3 ]]; } || \
   ...
   [[ -z ${swap_size} ]] || ! ls ${disks[@]} >/dev/null 2>&1; then
    help
    exit 1
fi
...
for disk in "${disks[@]}"; do
    ...
    if [[ ${first_partition_start} =~ ${integer_regex} ]] ; then
        parted --align optimal --script "${disk}" \
            ...
            "mkpart swap31415part $((first_partition_start + efi_system_partition_size)) $((first_partition_start + efi_system_partition_size + swap_size))" \
            "mkpart system31415part $((first_partition_start + efi_system_partition_size + swap_size)) ${system_size}" \
            "set 1 esp on"
    else
...
while read -r partition; do
    ...
    index=$((index+1))
done < <(find $(getPartitions 2) $(getPartitions 3))
...
# shellcheck disable=SC2046
if [ ${#disks[@]} -eq 1 ]; then
    swap_partition="$(getMapperPartitions 2)"
else
    swap_partition="/dev/md1"
    mdadm --create "${swap_partition}" --name swap31415md --level="${raid:-1}" --raid-devices=${#disks[@]} --metadata=default $(getMapperPartitions 2)
fi
...
# system partition
# shellcheck disable=SC2046
mkfs.btrfs --quiet --csum xxhash --data "${btrfs_raid}" --metadata "${btrfs_raid}" --label system31415fs $(getMapperPartitions 3)
...
# shellcheck disable=SC2046
mount -o noatime $(getMapperPartitions 3 | awk '{print $1}') /mnt/gentoo
...
umount /mnt/gentoo
# shellcheck disable=SC2046
mount -o noatime,subvol=@root $(getMapperPartitions 3 | awk '{print $1}') /mnt/gentoo
...
ln -s "$(getMapperPartitions 3 | awk '{print $1}')" /mnt/gentoo/mapperSystem
tmpCount=0
# shellcheck disable=SC2046
while read -r partition; do
